...
void LeePositionController::ComputeDesiredForce(Eigen::Vector3d* force_control_input)
{
	...
	{
		*force_control_input = - vehicle_parameters_.mass_ * vehicle_parameters_.gravity_ * e_3;
		safe_position = odometry_.position;
	}
	...
	else if(controller_mode_ == 2)
	{
		...
		*force_control_input = velocity_error.cwiseProduct(controller_parameters_.velocity_gain_)
